...
public class ClientDrawCanvas extends JFrame implements MouseMotionListener, MouseListener, Serializable { // runnable
	...
	public ClientDrawCanvas() {
		...
		mirrorPanel.setBackground(Color.red);
		mirrorPanel.setBounds(0, 0, 500, 500);	
		mirrorPanel.setVisible(false);
...
//		mirrorPane.add(drawCanvas); 					  // 그림판 캔버스 생성함
		...
		mirrorPanel.addMouseMotionListener(this); // mouse dragged시 위치 감지를 위해
		mirrorPanel.addMouseListener(this);
...
//			baos = new ByteArrayOutputStream();
		...
		try {
			...
			mirrorPanel.add(drawCanvas);
			PaintPacket pp = new PaintPacket(mirrorPanel);
			...
			// container은 말 그대로 콤포넌트 컨테이너 ㅠㅠ
			// 방법 없나요 ㅠㅠ

			os.close();	// finally로 빼서 if(os!=null) os.close()로 하는게 더 좋지만 일단 일케
			oos.close();
			bos.close();
			oos.close();
		} catch (IOException e1) {
	...
	public static void main(String[] args) { // 메인
		ClientDrawCanvas s1 = new ClientDrawCanvas();
	}
